...
export default function DynamicWizard(props: IDynamicWizardProps) {
  ...
  const done: boolean = useMemo(
    () => data.activeStages.length <= data.stageIndex,
    [data]
  );
  const lastStage: boolean = useMemo(
    () => data.activeStages.length - 1 <= data.stageIndex,
    [data]
  );
      ...
      <Switch>
        ...
        <Route
          ...
          render={(routeProps) => (
            <Stage0Array
              {...routeProps}
              data={{ activeStages: data.activeStages }}
              submit={onSubmit0}
              options={["a", "b", "c"]}
              lastStage={false}
            />
